...
city_list = {'london'};
parfor_progress('labeling', size(city_list, 1))
for i=1:size(city_list, 1)
    % load data
    dataset = [city_list{i,1},'_10_19'];
    load(['features/',features_type,'_test','/',features_type,'_', dataset,'.mat'],'routes');
    ...
    save(['features/',features_type,'_test','/',features_type,'_', dataset,'.mat'],'routes');
    parfor_progress('labeling');
end
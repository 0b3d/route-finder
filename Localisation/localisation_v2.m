...
bootstrapping_length = ones(test_num,1)*max_route_length_init; % default bootstrapping length
...
for i = 1:size(rs,2)
    r = rs(i);
    for j = 1:test_num
        if  r <= bootstrapping_length(j)
            t = test_route(j,1:r); % 1:15, 2:16,3:17,4:18,5:19,6:20
        else
            t = test_route(j, r-bootstrapping_length(j)+1:r);    
        end
    ...
    for j = 1:size(distance_thresholds, 2)
        ...
        for p = 1:test_num
            ...
            if ~isempty(best_estimated_routes{1,p}{1,r})
                pred_index = best_estimated_routes{1,p}{1,r}(end);
...
if strcmp(features_type, 'ES')
    if strcmp(dataset,"cmu5k")
        resultsPath = ['results_bs/', features_type, '/', model, '/', tile_test_zoom, '/', dataset,'_',subset];
    else
        resultsPath = ['results_bs/', features_type, '/', model, '/', tile_test_zoom, '/', dataset];
    end
    ...
    if strcmp(dataset,"cmu5k")
        resultsPath = ['results_bs/', features_type,'/',dataset,'_',subset];
    else
        resultsPath = ['results_bs/', features_type,'/',dataset];
    end
    ...
    % real classifier
    save([resultsPath,'/', option,'_', network,'_v2','.mat'],  '-v7.3')
    % simulated classifier

function panos = getTopology(panos, dists, az, inters, ways, radius, opts)
...
opts.straightThresh = radius;
opts.minInterDist = 1;%0 % minimum allowed to be topologically an intersection
%opts.maxInterDist = 100; % initial thresholding for assigning intersections to panos
opts.maxInterDist = radius; % meters
...
[panos.forward_inter] = unfold(num2cell(NaN(length(panos),2),2));
[panos.backward_inter] = unfold(num2cell(NaN(length(panos),2),2));
...
%             direct = 1; % forward
%         end
        if(adjustedAZ > 135 && adjustedAZ < 315)
            direct = 2; % backward
        elseif((adjustedAZ > 0 && adjustedAZ < 45) || (adjustedAZ > 315 && adjustedAZ < 360))
            direct = 1; % forward
        end
    ...
    for direct = 1:2
        ...
        elseif(min_fb_dist(direct) < opts.minInterDist || min_fb_dist(direct) > opts.maxInterDist)
            ...
            if direct == 1
                panos(i).forward_inter = inters(j).coords;
            else
                panos(i).backward_inter = inters(j).coords;
            end      
        end

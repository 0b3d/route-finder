function [rank, best_routes, best_top5_routes, route_dist, bootstrapping_length] = RouteSearching_ES_bs(routes, N, max_route_length, threshold, R_init, t, T, turns, pairwise_dist, overlap, s_number)
R = R_init;
...
for m=1 : max_route_length
    ...
    if m == 1
        [R_, dist_] = Nclosest_es(t(m),R,dist,N(m),pairwise_dist); % filter based on sorting
        t_ = R_(1,:);
        ...
        if size(R_, 1) > 0
            if strcmp(turns, 'true')
                ...
                if ~flag_bs
                    ...
                    [R_, dist_] = Nclosest_es(t(m),R_,dist_,N(m),pairwise_dist); % filter based on sorting
                    t_ = R_(1,:);
...
%                     end
                    ...
                    [R_, dist_] = Nclosest_es(t(m),R_,dist_,100,pairwise_dist); % N = 100
                    if ~isempty(R_)
                        t_ = R_(1,:);
                        min_dist = dist_(1);
                    end
...
%                     end
                    ...
                    [R, dist] = boot_strapping(t_, min_dist, routes, bootstrapping_length);
                    [R_, dist_] = Nclosest_es(t(m),R,dist,100,pairwise_dist); % filter based on sorting
                    if ~isempty(R_)
                        t_ = R_(1,:);
                        min_dist = dist_(1);
                    end

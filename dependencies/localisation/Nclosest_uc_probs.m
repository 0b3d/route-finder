function [R_, probs_] = Nclosest_uc_probs(y,R,routes,probs,N,t,m, pairwise_probs)
sz1 = size(R, 1);
sz2 = size(R, 2);
...
for i=1:sz1      % slow
    ...
    if ~isempty(routes(k).x)
        ...
        yx_probs = 1.0 - pairwise_probs(t(m),k); 
...
%             xx_probs = normpdf(abs(xx_dist - yy_dist), 0.0, 0.789364);

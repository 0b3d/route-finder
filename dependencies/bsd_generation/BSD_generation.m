function panos = BSD_generation(panos, inters, buildings, radius, range, thresh_jc, thresh_bd, thresh_dist)
%% get BSD directly
...
for p=1:size(panos, 2)
    ...
    descList = zeros(1, 4);
    flabel = getLabelJC(search_areas.forward, junctions_in_circle, locaCoords, thresh_jc);
    descList(1) = flabel;
    
    blabel = getLabelJC(search_areas.backward, junctions_in_circle, locaCoords, thresh_jc);  
    descList(3) = blabel;
    
    %% Get Labels for gaps
    llabel = getLabelBD(search_areas.left, buildings_in_circle, locaCoords, thresh_bd, radius, thresh_dist);
    descList(4) = llabel;

        
    rlabel = getLabelBD(search_areas.right, buildings_in_circle, locaCoords, thresh_bd, radius, thresh_dist);
    descList(2) = rlabel;
